...
const MobileMenu = ({ drawerOpen, toggleDrawer, categories }) => {
    return (
        <>
   <Box sx={{ display: { xs: 'block', md: 'none' }, position: 'fixed', bottom: 10, left: 10, width: '50px', height: '50px', backgroundColor: '#007aff', boxShadow: '0 -2px 5px rgba(0,0,0,0.1)', borderRadius: 1, zIndex: 1300 }}>
        <IconButton onClick={toggleDrawer(true)} sx={{ width: '100%', color: '#fff' }}>
          <Menu sx={{ fontSize: 30 }} />
        </IconButton>
      </Box>
